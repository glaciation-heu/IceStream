...
async fn main() {
    ...
    println!("Starting client service on {}:{}", addr, port);

    let api_service =
        OpenApiService::new(api::Api, "CarbyneStack Client Service", "1.0")
            .description("Client Service to access the computation service. There will be 3 access-scopes: input_party, output_party, coordinator")
